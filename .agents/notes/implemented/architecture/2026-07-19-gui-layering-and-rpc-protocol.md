...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - The Electron application reuses the same web client packages. Its first
      bootstrap uses the Web carrier on an OS-assigned loopback port; the
      production-hardening target remains an IPC fetch carrier, as recorded in
      the [desktop bootstrap note](2026-08-14-electron-desktop-bootstrap.md).
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser access plus the Electron bootstrap carrier; zero workspace dependencies (the registry arrives by structural injection); IPC is the desktop hardening target |
...
| IPC bridge subclass (planned desktop hardening — not implemented yet) | the Electron shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
