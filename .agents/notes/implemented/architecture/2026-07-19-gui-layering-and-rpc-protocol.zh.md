...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - Electron 应用复用同一套 web client 包。首个启动版本经由操作系统分配的回环端口
      使用 Web 载体；生产硬化目标仍为 IPC fetch 载体，详见
      [桌面端启动说明](2026-08-14-electron-desktop-bootstrap.md)。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器访问与 Electron 首版启动载体；零 workspace 依赖（注册表经结构注入到达）；IPC 是桌面端硬化目标 |
...
| IPC 桥子类（已规划的桌面端硬化——尚未实现） | Electron 壳 | IPC 序列化往返 | 只需换 doFetch，约定/基类零改 |
