...
[`wrangler.jsonc`](wrangler.jsonc) 为 Cloudflare Workers 和 Static Assets 配置 TanStack Start 服务端入口。完成 Cloudflare 身份验证后，`pnpm --filter @deepseek-ai/dsh-site deploy` 会通过 Wrangler 构建并部署网站。
