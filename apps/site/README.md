...
[`wrangler.jsonc`](wrangler.jsonc) configures the TanStack Start server entry, Static Assets, and the production custom domain `deedoo.willhong.dev` for Cloudflare Workers. `pnpm --filter @deepseek-ai/dsh-site deploy` builds and deploys directly through Wrangler after Cloudflare authentication. Cloudflare is the default and owning host; ChatGPT Sites is not a deployment fallback unless the user explicitly requests it.
