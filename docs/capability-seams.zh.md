...
flowchart LR
  ...
  pkg_llm_vision_sidecar["llm-vision-sidecar"]
  svc_visionSidecar["ctx.visionSidecar<br/>Text-route vision preparation"]
  ...
  pkg_llm_vision_sidecar --> svc_visionSidecar
  pkg_lsp --> svc_lsp
  ...
  svc_visionSidecar --> pkg_host_apiproxy
  svc_visionSidecar --> pkg_tool_fs
...
| `ctx.visionSidecar` | `core` | [`llm-vision-sidecar`](../packages/llm/llm-vision-sidecar) | - | [`host-apiproxy`](../packages/host/apiproxy), [`tool-fs`](../packages/fs/tool-fs) | - | 通过一条已配置的图片路由，在纯文本 agent 步骤之前描述尚未处理的持久图片；Host 准入和 read_image 会查询其可选能力接口。 |
