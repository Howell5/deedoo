...
`read_image` registers only while a durable `ctx.attachments` service is mounted — without one the deployment cannot commit image bytes, so the tool never appears. Execution requires the exact routed model to declare `image` input or an available `llm-vision-sidecar` (resolved through `ctx.llm.resolveModelInfo` from the session's latest request header, falling back to agent options); an unknown or unsupported route without a sidecar gets a refusal before any filesystem I/O.
...
| `read_image` | `file_path` | Reads a PNG/JPEG/WebP/GIF file through the bounded byte seam, persists it through `ctx.attachments.saveImage`, and returns an image block beside a small metadata envelope. It succeeds when the exact route declares image input or a configured vision sidecar can describe it. |
...
A successful `read_image` returns `<path><displayPath></path>`, `<type>image</type>`, and a `<content>` envelope naming the media type, dimensions, and byte size, followed by the image itself as a native image block. On a text-only route, `llm-vision-sidecar` adds a durable description before the next model request. The session log stores the durable `sha256:` attachment reference and optional description; the routed provider re-reads and digest-verifies the bytes when its adapter uses native image input.
...
- **The route gate races a concurrent model switch** — `read_image` checks the latest routed model and sidecar admission at execution; a switch committed before the next pre-step can select a different sidecar or leave the image request without one. The Web host applies the same native-or-sidecar rule when admitting prompts and model switches.
