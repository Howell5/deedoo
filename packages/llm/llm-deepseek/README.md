...
The selected DeepSeek model receives the harness system prompt, message history, tool schemas, stop sequences, and call config without adapter-authored prompt prose. A `llm-vision-sidecar`-enriched image is represented by its bounded `[Image description]` text; an unresolved image still fails with `UNSUPPORTED_CONTENT`. On a prior assistant turn with tool calls, its reasoning content is passed back as required; reasoning from tool-call-free turns is omitted.
...
- **Serialization flattens user and tool-result content to text blocks** — sidecar-enriched images contribute their persisted description, other plugin-added block types are skipped, and empty tool output crosses the wire as the literal `(no output)`.
