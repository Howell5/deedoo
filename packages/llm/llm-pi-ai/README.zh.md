...
`[text]` 是「尚未声明」，而不是对端点的猜测——这也是为什么这里的回退值取保守值，而两个容量回退值只是取一个说得过去的值。这里没有任何环节会去询问网关实际接受什么，而两种猜错的代价并不对等：没有视觉旁路时，模态中不含图片会让 Harness 在图片被附加之前就拒绝；而多声明会接纳一张图片、再由提供方在轮次中途拒绝——此时消息已经持久化，会话便会不断重复一个不可能成功的请求。`@deepseek-ai/dsh-llm-vision-sidecar` 是独立的准备层，供部署将这些图片明确路由到另一条已声明的图片能力模型。
...
- **模态声明不经验证，且多声明的后果超出本轮**：没有任何环节会去询问端点接受什么，因此声明了网关并不提供的 `image` 的模型不会在这里被拦下，而是由提供方在轮次中途拒绝。prompt 准入在构造请求之前就把用户消息持久化提交；视觉旁路只有在自身声明的图片路由可用时才会为纯文本路由规避这一失败。未处理的图片会 fail-closed，而错误的原生声明仍可能让持久图片被模型不断重发。恢复途径是换一个确实支持图片的模型、配置视觉旁路、fork 到图片之前，或开启新会话；发送失败时把尚未消费的图片消息从日志中回滚出去这件事已暂缓。
