...
`[text]` is the absence of a declaration rather than a guess at the endpoint, which is why the fallback here is conservative where the capacity fallbacks are merely plausible. Nothing interrogates a gateway for what it accepts, and the two wrong answers do not cost the same: without a vision sidecar the harness refuses an image before it is attached when a model's modalities omit one, while over-claiming admits an image the provider then rejects mid-turn — after the message is durable, which leaves the session repeating a request that cannot succeed. `@deepseek-ai/dsh-llm-vision-sidecar` is the separate preparation layer for deployments that deliberately route those images through another declared image-capable model.
...
- **A modality declaration is not verified, and over-claiming outlives the turn** — nothing interrogates an endpoint for what it accepts, so a model declaring `image` its gateway does not serve is refused by the provider mid-turn rather than here. Prompt admission commits the user message durably before the request is built; the vision sidecar avoids that failure for a text-only route only when its own declared image-capable route is available. An unprocessed image remains fail-closed, while a bad native declaration can still leave a durable image the model keeps re-sending. Recovery is another image-capable model, a configured sidecar, a fork before the image, or a new session; rolling an unconsumed image message back out of the log on a failed send is deferred.
